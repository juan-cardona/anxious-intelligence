/**
 * Orchestrator — the main interaction loop with agentic tool use.
 */
...
import { formatBeliefsForPrompt, systemPromptWithBeliefs } from "./prompts.js";
...
export async function processInteraction(
  ...
  // 4. Generate response — with or without tools
  let response: string;
  ...
  if (useTools) {
    ...
    response = await callClaude(system, userMessage);
  }
