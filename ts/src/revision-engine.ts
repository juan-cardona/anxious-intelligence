...
async function discoverConnections(
  ...
  const validRelations = new Set<string>([
    "supports", "contradicts", "depends_on", "generalizes", "tension_shares",
  ]);
...
export async function reviseBelief(
  ...
  const storedTexts = storedConnected.map(
    (b) => `[stored] ${b.content} (conf=${b.confidence.toFixed(2)}, tension=${b.tension.toFixed(2)})`,
  );
  ...
  for (const disc of discovered) {
    await connectBeliefs(
      belief.id, disc.belief_id, disc.relation, disc.strength,
      "llm_revision", disc.reasoning.slice(0, 500),
    );
  ...
  await query(
    `INSERT INTO revisions (old_belief_id, new_belief_id, trigger_tension, evidence_summary, cascaded_beliefs, reasoning)
     VALUES ($1, $2, $3, $4, $5, $6)`,
    [belief.id, newBelief.id, belief.tension,
     contradictionTexts.slice(0, 10).join("\n"), cascadedIds, result.reasoning ?? ""],
  );
    ...
    for (const conn of allConnected.values()) {
      if (conn.content.includes(ci.belief ?? "")) {
        ...
        if (updated && updated.tension >= REVISION_THRESHOLD) {
          cascadeResults.push(await reviseBelief(updated, depth + 1));
...
export async function reviseAllTriggered(beliefs: Belief[]): Promise<RevisionResult[]> {
...
export async function getRecentRevisions(limit = 5): Promise<Array<Record<string, any>>> {
  return queryMany(
